...
\begin{figure}[ht]
\centering
\includegraphics[width=.50\textwidth]{figs/fig_arquitetura.png}
\caption{Arquitetura do solver híbrido: dispatcher com decisor SA, pool de
...
\begin{figure}[ht]
\centering
\includegraphics[width=.52\textwidth]{figs/fig_telemetria.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=.54\textwidth]{figs/fig_modos.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=.58\textwidth]{figs/fig_campanhas.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=.58\textwidth]{figs/fig_megabatch.png}
...
\begin{figure}[ht]
\centering
\includegraphics[width=.75\textwidth]{figs/fig_5metodos.png}
...
(Figura~\ref{fig:classes}(a)). A Figura~\ref{fig:classes}(b) posiciona
esses números frente aos tempos publicados do SP Classe C completo
(benchmark inteiro, incluindo RHS e comunicação), de 29,3~s (Titan) a
483,24~s (serial): a comparação é por fase, não por benchmark — mas indica
que a fase dominante cabe em segundos em um único nó heterogêneo, escala
que a especificação original do NPB projetou para execução em sistemas
massivamente paralelos de 1000+ processadores \cite{bailey1993nas}.

\begin{figure}[ht]
\centering
\includegraphics[width=.58\textwidth]{figs/fig_classes.png}
...
\textbf{Democratização do HPC e eficiência de recursos.}
Tradicionalmente, a execução da Classe E do NPB SP
($1020^3 \approx 1{,}06\times 10^9$ pontos de malha) é reservada a
supercomputadores com centenas de nós interconectados por redes de baixa
latência. A abordagem híbrida desenvolvida demonstra que a otimização
algorítmica orientada à microarquitetura contorna a escassez de
infraestruturas massivas: combinando a lista livre $O(1)$, a pré-fatoração
LU estacionária e o ajuste estocástico on-line por SA, foi possível
processar mais de 1,061 trilhão de pontos em 213,9~s em um único nó de
trabalho. O gargalo de memória e latência característico de matrizes
bandadas não exige, portanto, escalonamento horizontal em larga escala
(força bruta de hardware), mas sim a exploração cooperativa e balanceada
dos barramentos locais (RAM e VRAM) — democratizando malhas extremas em
estações de trabalho convencionais.

\textbf{Limitações.} (i) A comparação GFLOPS com o NPB-GPU usa contagens de
flops diferentes (28 vs. ~850 flops/ponto) e o NPB-GPU de referência compila
com \texttt{-use\_fast\_math}, enquanto o solver proposto usa aritmética
estrita; métricas invariantes (pontos/s, sistemas/s) são as recomendadas
para citação. (ii) A margem de 13\% sobre o modo somente-GPU é específica
desta máquina; o ganho estrutural garantido é sobre o modo somente-CPU
(3,7$\times$). (iii) O run cobre uma única direção da varredura por iteração;
a extensão às varreduras y e z (e à avaliação do RHS) é trabalho futuro.

\section{Conclusão e trabalhos futuros}\label{sec:conc}

Este trabalho apresentou um solver pentadiagonal híbrido CPU--GPU com
balanceamento adaptativo por simulated annealing, garantia de
reprodutibilidade bit-a-bit e monitoramento térmico via NVML. Em uma
plataforma de nó único (1 CPU + 1 GPU), o solver atingiu 4,86 milhões de
sistemas lineares por segundo e resolveu 1,061 trilhão de pontos com
checksum estritamente idêntico, superando tanto o modo puramente GPU
quanto o puramente CPU na mesma máquina.

A principal contribuição prática reside na viabilização de simulações de
malha fina em escala extrema em ambientes computacionais restritos,
dispensando calibrações manuais (zero-profiling) e atenuando o gargalo de
transferência PCIe: enquanto a GPU consome sua fatia na VRAM, a CPU processa
dados residentes na RAM do host, escondendo a latência de transferência.

Trabalhos futuros contemplam: a extensão às três varreduras e ao passo
completo do NPB SP (permitindo comparação direta da contagem oficial de
flops); a expansão do modelo para nós multi-GPU com o mesmo decisor SA; a
inclusão de restrições de eficiência energética na função de custo do
otimizador; e a avaliação do esquema em outras cargas de banda estreita.
