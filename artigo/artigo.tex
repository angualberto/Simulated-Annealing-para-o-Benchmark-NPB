...
\begin{abstract}
  The pentadiagonal sweep of the NPB SP benchmark is memory-bound. We present a hybrid CPU--GPU solver with dynamic load balancing via simulated annealing (SA). On an RTX 5000 Ada, the solver processed 1.061 trillion points in 213.9~s (163~ms/sweep; 182 GFLOPS), outperforming pure-GPU (185~ms) and pure-CPU (611~ms) modes and reaching 4.86M systems/s. Under severe Out-of-Core conditions (legacy GTX 660 2~GB with SSD swap), the \textit{Edge Transpose} technique and SA filtering allowed solving the 1.06-trillion-point mesh in 90.87~s ($X$), 91.98~s ($Y$), and 212.43~s ($Z$), preserving bit-identical checksums.
\end{abstract}

\begin{resumo}
  A varredura pentadiagonal do benchmark NPB SP é limitada por memória. Apresentamos um solver híbrido CPU--GPU com balanceamento dinâmico via \textit{Simulated Annealing} (SA). Na RTX 5000 Ada, o solver processou 1,061 trilhão de pontos em 213,9~s (163~ms/varredura; 182 GFLOPS), superando a GPU pura (185~ms) e a CPU pura (611~ms) atingindo 4,86 milhões de sistemas/s. Em ambiente \textit{Out-of-Core} legado (GTX 660 2~GB com swap SSD), a técnica \textit{Edge Transpose} e a filtragem SA resolveram a malha de 1,06 trilhão de pontos em 90,87~s ($X$), 91,98~s ($Y$) e 212,43~s ($Z$), preservando o \textit{checksum} invariante.
\end{resumo}
...
casos de interesse físico. Um pôster técnico da NASA \cite{nasa:2021} demonstrou ganhos de 92\% sobre a NVIDIA, atribuindo o ganho à menor demanda de banda de memória. Enquanto PfSolve \cite{pfsolve:2025} e bibliotecas Multi-GPU (PaScaL\_TDMA \cite{kang:2023}, Pipelined-TDMA \cite{kim:2025}) focam em escalabilidade horizontal de hiper-grafos, o solver SPIKE \cite{chang:2012} estabelece a base de estabilidade numérica. Nossa abordagem contrapõe frameworks de nó único como StarPU \cite{starpu:2011}.
...
\begin{equation}
\begin{aligned}
  l_{i,i-2} &= a_i/u_{i-2,i-2}, \quad
  l_{i,i-1} = (b_i - l_{i,i-2}\,u_{i-2,i-1})/u_{i-1,i-1},\\
  u_{i,i}   &= c_i - l_{i,i-1}\,u_{i-1,i} - l_{i,i-2}\,u_{i-2,i}, \quad
  u_{i,i+1} = d_i - l_{i,i-1}\,u_{i-1,i+1}, \quad
  u_{i,i+2} = e_i,
\end{aligned}
\label{eq:fatoracao}
\end{equation}
para $i = 1, \dots, n$, com as condições de contorno tratadas separadamente. A solução de $Ax = f$ segue por substituição direta e retrosubstituição ($y_i = f_i - l_{i,i-1}\,y_{i-1} - l_{i,i-2}\,y_{i-2}$ e $x_i = (y_i - u_{i,i+1}\,x_{i+1} - u_{i,i+2}\,x_{i+2})/u_{i,i}$). 

Cada linha exige 28 operações em ponto flutuante por ponto \cite{golub:2013}. A varredura \texttt{solve\_lines\_kernel} elimina a sub-diagonal (Eq. \ref{eq:fatoracao}) seguida da substituição para obter $x$. As recorrências são idênticas nas CPUs e GPU, invocadas com \texttt{-fmad=false} no host para evitar contrações matemáticas que introduzam diferenças de arredondamento.
...
\begin{minipage}{0.48\textwidth}
  \centering
  \includegraphics[width=0.75\linewidth]{figs/fig_telemetria.png}
  \caption{Evolução do run de 1000 iterações: tempo por varredura e temperatura da GPU.}
  \label{fig:telemetria}
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}
  \centering
  \includegraphics[width=0.75\linewidth]{figs/fig_modos.png}
...
\begin{minipage}{0.48\textwidth}
  \centering
  \includegraphics[width=0.75\linewidth]{figs/fig_campanhas.png}
  \caption{(a) EMA vs.\ SA; (b) fração forçada (SA automático em tracejado).}
  \label{fig:campanhas}
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}
  \centering
  \includegraphics[width=0.75\linewidth]{figs/fig_megabatch.png}
  \caption{(a) Lotes por modo; (b) cuPentBatch vs solver (n=1020).}
  \label{fig:megabatch}
\end{minipage}
\vspace{-2mm}
\end{figure}
...
\begin{minipage}{0.48\textwidth}
  \centering
  \includegraphics[width=0.75\linewidth]{figs/fig_5metodos.png}
  \caption{Tempo mínimo por varredura dos 5 métodos avaliados no mega-lote.}
  \label{fig:5metodos}
\end{minipage}\hfill
\begin{minipage}{0.48\textwidth}
  \centering
  \includegraphics[width=0.75\linewidth]{figs/fig_classes.png}
  \caption{(a) Projeção C/D/E; (b) benchmark publicado vs. fase (asterisco).}
  \label{fig:classes}
\end{minipage}
\vspace{-2mm}
\end{figure}
...
\noindent\textbf{Eficiência de Recursos.} O gargalo de memória de matrizes bandadas não exige escalonamento horizontal massivo. A combinação de lista livre, fatoração LU e ajuste por SA alcança vazões de trilhões de pontos em nós de trabalho únicos.

\subsection{Resiliência Out-of-Core e Varreduras Tridimensionais}
Na execução da Classe E ($16{,}8$~GB) num nó legado de 15~GiB e GTX 660 2~GB (swap SSD), a Média Móvel Exponencial (EMA) sofreu severamente para interpretar picos não-determinísticos de I/O da paginação. O EMA inflacionou a latência histórica da CPU para $266{,}8$~ms/bloco, o dobro do SA ($138{,}9$~ms/bloco), distorcendo a fração de despacho. O \textit{Simulated Annealing} (SA) provou matematicamente filtrar esse ruído em sua janela de energia, contornando o afogamento (Tabela~\ref{tab:super_ooc}).

\begin{table}[htbp]
\centering
\caption{Resiliência Out-of-Core (swap SSD). O EMA falha sob thrashing; o SA converge.}
\label{tab:super_ooc}
\scriptsize
\begin{tabular}{lcccc}
\toprule
\textbf{Eixo (Modo)} & \textbf{Tempo Varredura} & \textbf{Ruído CPU (Latência)} & \textbf{Split ($\alpha$)} & \textbf{Vazão SSD} \\
\midrule
$X$ (EMA Baseline) & 94,12 s & 266,89 ms/bloco & 36\% GPU / 63\% CPU & -- \\
$X$ (SA Nativo) & \textbf{90,87 s} & \textbf{138,97 ms/bloco} & 39\% GPU / 61\% CPU & 186,6 MB/s \\
$Y$ (SA Transpose) & \textbf{91,98 s} & estável & 42\% GPU / 58\% CPU & 184,4 MB/s \\
$Z$ (SA Transpose) & \textbf{212,43 s} & afogamento I/O detectado & 21\% GPU / 79\% CPU & 79,8 MB/s \\
\bottomrule
\end{tabular}
\end{table}

Para as varreduras $Y$ e $Z$ (onde os dados sofrem enormes saltos, ocasionando \textit{page faults} apocalípticos no SSD), aplicou-se a \textit{Edge Transpose}: as \textit{threads} da CPU agiram como um tradutor I/O, agrupando os pontos dispersos em um bloco linear de 16~MB e o enviando à GPU. A varredura $Y$ atingiu desempenho virtualmente idêntico à nativa $X$ graças ao empacotamento paralelo.

\noindent\textbf{A asfixia I/O e a intervenção do SA.}
Já a varredura $Z$ (saltos astronômicos de 8~MB/ponto) testou os limites físicos da controladora SSD. O brilho da execução está no \textit{Simulated Annealing}: o algoritmo detectou que a \textit{thread} despachante da GPU estava sendo asfixiada pela barreira física do I/O e, de maneira completamente autônoma, inverteu o fluxo. Jogando 79\% da carga matemática para os \textit{workers} da CPU (\textit{zero-profiling}), o SA superou o gargalo físico do armazenamento virtual e finalizou a tarefa em 212 segundos.
