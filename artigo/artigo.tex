...
\begin{abstract}
  The scalar pentadiagonal solver of the NAS Parallel Benchmark (NPB SP) is a memory-bound workload. We present a hybrid CPU--GPU solver with runtime load balancing via simulated annealing (SA). Validated by bit-identical checksums (1000 iterations) and NVML thermal monitoring, the solver processed 1,061,191,680,000 points (1.04 billion systems of order 1020) in 213.9~s on an RTX 5000 Ada. At 163~ms/sweep and 182 GFLOPS, it surpassed pure-GPU (185~ms) and pure-CPU (611~ms) modes. It achieved 4.86M systems/s, comparable to the official NPB-GPU CUDA reference (200--216 GFLOPS).
\end{abstract}

\begin{resumo}
  A varredura pentadiagonal do benchmark NPB SP é limitada por memória. Apresentamos um solver híbrido CPU--GPU ajustado dinamicamente via \textit{simulated annealing} (SA). Validada por \textit{checksums} bit-idênticos (1000 iterações) e termometria NVML, a execução numa RTX 5000 Ada resolveu 1.061.191.680.000 pontos (1,04 bilhão de sistemas de ordem 1020) em 213,9~s (163~ms/varredura; 182 GFLOPS). O modelo superou os modos somente-GPU (185~ms) e somente-CPU (611~ms), alcançando 4,86 milhões de sistemas/s, desempenho na mesma ordem da referência NPB-GPU CUDA (200--216 GFLOPS).
\end{resumo}
