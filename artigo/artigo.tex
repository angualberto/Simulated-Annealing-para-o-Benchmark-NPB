...
Na execução da Classe E ($16{,}8$~GB) num nó legado de 15~GiB e GTX 660 2~GB (swap 32~GiB SSD), a paginação gerou \textit{thrashing}. O controlador EMA acumulou esse ruído (inflando a latência da CPU para $266{,}89$~ms/bloco). O decisor SA filtrou os picos transitórios, contornando o afogamento (Tabela~\ref{tab:super_ooc}).
...
Para transpor os saltos descontínuos em $Y$ e $Z$, as threads \textit{host} linearizaram blocos em micro-buffers de 16~MB. Em $Y$, operou na taxa de $X$. Em $Z$ (salto de 8~MB/ponto), saturou I/O para 79,8~MB/s; o SA detectou a contenção e deslocou 79\% para a CPU, concluindo a malha 3D.
