...
Tradicionalmente, a execução da Classe E do NPB SP ($1020^3 \approx 1{,}06\times 10^9$ pontos de grade) é associada a infraestruturas de supercomputação com centenas de nós interconectados por redes de baixa latência~\cite{bailey1993nas}. A arquitetura híbrida proposta demonstra que a otimização de baixo nível combinada ao controle estocástico on-line permite contornar a escassez de hardware de ponta: ao unir a lista livre $O(1)$, pré-fatoração LU estacionária e ajuste adaptativo por SA, foi possível computar mais de 1,061 trilhão de pontos espaço-temporais em 213,9~s no nó EPYC/RTX 5000 Ada~\cite{araujo2021nas} e viabilizar a mesma malha em uma estação legada em 90,87~s. Esse resultado evidencia que o gargalo de memória de matrizes bandadas não requer obrigatoriamente força bruta de hardware distribuído, mas sim a exploração balanceada da hierarquia local de memória (RAM e VRAM).

\noindent\textbf{Resiliência Dinâmica a Jitter e Sobrecarga.}
Em contrapartida a runtimes baseados em grafos de tarefas com custo de escalonamento proporcional ao número de dependências~\cite{augonnet2011starpu}, o despachante com janela deslizante avalia o custo real medido $E(x)$ instantâneo. Em cenários de oscilação térmica ou contenção de I/O na CPU, a aceitação probabilística de Metropolis evita que a aplicação estagne aguardando threads atrasadas (\textit{stragglers}), ajustando a fração $\alpha$ de forma autônoma sem requerer perfis analíticos estáticos prévios.
