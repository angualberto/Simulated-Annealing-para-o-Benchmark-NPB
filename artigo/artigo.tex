...
A robustez do despachante adaptativo e do mecanismo \textit{Edge Transpose} foi avaliada na malha tridimensional completa da Classe E ($1020^3 \approx 1{,}06\times 10^9$ pontos, demandando $\sim$16,8~GB) em ambiente de estresse extremo: CPU AMD FX-6300 (3 núcleos ativos, 15~GiB RAM) e GPU legada NVIDIA GeForce GTX 660 (2~GB VRAM, arquitetura Kepler \texttt{sm\_30}) paginando em swap SSD de 32~GiB.

\begin{table}[htbp]
\centering
\caption{Desempenho Out-of-Core e comportamento do SA nas três direções espaciais (malha $1020^3$, 4064 blocos, GTX 660 2~GB sob swap SSD).}
\label{tab:3d_ooc}
\footnotesize
\begin{tabular}{lcccc}
\hline
\textbf{Eixo Espacial} & \textbf{Padrão de Acesso} & \textbf{Tempo Total (s)} & \textbf{Split ($\alpha$)} & \textbf{Checksum Bit-a-Bit} \\
\hline
Eixo $X$ (Nativo)      & Contíguo                  & \textbf{90,87}           & 39\% GPU / 61\% CPU       & $1{,}889440\times 10^8$ \\
Eixo $Y$ (Transposto)  & Stride $N$ ($8{,}16$~kB)  & \textbf{91,98}           & 42\% GPU / 58\% CPU       & $1{,}889440\times 10^8$ \\
Eixo $Z$ (Transposto)  & Stride $N^2$ ($8{,}32$~MB)& \textbf{212,43}          & 21\% GPU / 79\% CPU       & $1{,}889440\times 10^8$ \\
\hline
\end{tabular}
\end{table}

Na comparação direta com o controlador por Média Móvel Exponencial (EMA) no eixo $X$, o EMA sofreu com picos estocásticos de paginação do disco, reportando ruído de 266,89~ms/bloco na CPU e tempo total de 94,12~s. O otimizador SA filtrou o ruído de I/O em sua janela de energia ($W=16$), estabilizando a latência em 138,97~ms/bloco e reduzindo a varredura para 90,87~s. 

Para os eixos multidimensionais (Tabela~\ref{tab:3d_ooc}), o \textit{Edge Transpose} permitiu ao eixo $Y$ atingir tempo equivalente ao nativo ($91{,}98\text{ s}$ vs. $90{,}87\text{ s}$). No eixo $Z$, sob forte contenção da controladora SSD pelo salto de $N^2$, o controlador SA detectou em tempo real a asfixia de I/O no despachante e ajustou autonomamente o split para 79\% CPU / 21\% GPU, concluindo a varredura em 212,43~s com checksum rigorosamente bit-idêntico.
