...
static void* gpu_dispatcher(void* arg_) {
    ...
    // por lote de BATCH blocos (reduz o overhead por lancamento em ~MEGA/BATCH).
    // A decisao por bloco (EMA/SA/top-up) permanece identica; so o AGENDAMENTO
    // dos lancamentos muda. Flush tambem ao fim da varredura.
    long npend = 0;
