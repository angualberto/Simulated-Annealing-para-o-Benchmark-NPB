...
// reutilizado de /home/andre/Documentos/hpc/sp_class_e.f90 e sp_class_e_cuda.cu.
//
// Arquitetura de despacho IDENTICA ao hybrid_hlist.cu:
//   Fila = LISTA LIGADA HIERARQUICA COM INDICE (indice + listas por nivel):
//     Nivel 1 - Dispositivos: devices[] (0=LIVRE, 1=CPU, 2=GPU)
//     Nivel 2 - Periodos:     INDEX periods[pid] (O(1))  +  lista dupla de periodos
//     Nivel 3 - Blocos:       INDEX blocks[idx]  (O(1))  +  lista dupla de blocos
//   dispatcher (dono do LIVRE) popa blocos, decide GPU/CPU pela resposta medida, move O(1)
//
...
//
// Compilar: nvcc -O3 -arch=sm_89 -o hybrid_hlist_sp hybrid_hlist_sp.cu -lpthread
// Rodar:    ./hybrid_hlist_sp [n] [json] [nb_blocos] [linhas_por_bloco]
...
// transformado y DIRETAMENTE em u (q sera sobrescrito pela retrosubstituicao).
...
// de referencia (verificado: checksum 3.4645900000e+07 bit-identico entre
// all-GPU, all-CPU e todos os splits).
// O bug original usava b[base+i] SEM armazenar y na retrosubstituicao →
// solucao errada (~28% de erro no checksum).
...
static void* gpu_dispatcher(void* arg_) {
    ...
    // em lotes de MEGA blocos (1 kernel + 1 sync), em vez de 1 kernel + 1 sync
    // por lote de BATCH blocos. A decisao por bloco (EMA/top-up) permanece
    // identica; so o AGENDAMENTO dos lancamentos muda. Flush tambem ao fim.
    long npend = 0;
    ...
    while (livre.head) {
        ...
        if (npend >= MEGA) launch_pending();   // mega-lote: 1 kernel p/ MEGA blocos
        (void)nb2;
...
int main(int argc, char** argv) {
    ...
    printf("\n=== LISTA LIGADA HIERARQUICA COM INDICE + PROBLEMA REAL (SP pentadiagonal) ===\n");
    ...
    js << "{\n";
    js << "  \"experimento\": \"Lista ligada hierarquica com indice + problema real SP pentadiagonal\",\n";
